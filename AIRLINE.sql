...
CREATE TRIGGER GIAVE  ON VE
	...
	AS
		BEGIN
			...
			DECLARE @HANGVE SMALLINT
			DECLARE @MACHUYENBAY INT
			DECLARE @GIAVEGOC MONEY
			DECLARE @GIAVE2 MONEY

			SELECT @HANGVE = HANGVE FROM INSERTED
			SELECT @MACHUYENBAY = MACHUYENBAY FROM INSERTED
			SELECT @GIAVEGOC= GIAVEHANG1 FROM CHUYENBAY WHERE MACHUYENBAY = @MACHUYENBAY
			SELECT @GIAVE2 =GIAVEHANG2 FROM CHUYENBAY WHERE MACHUYENBAY=@MACHUYENBAY
			IF (@HANGVE = 1)
				BEGIN
					UPDATE VE
					SET @GIAVEHANG1 = @GIAVEGOC
				END
			ELSE
				BEGIN
					UPDATE VE
					SET GIAVE = @GIAVEGOC*1.05
				END	
